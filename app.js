...
function summaryImageGenreCard(genre, count, color) {
  return `
    <div style="min-width:100px;height:auto;padding:6px 16px 28px;border-radius:14px;display:flex;flex-direction:column;align-items:center;box-shadow:0 2px 8px rgba(139,111,71,0.12);border:1px solid rgba(0,0,0,0.06);background:${color.bg};color:${color.text};">
  ...
  return `
    <div style="background:#ffffff;border:1px solid #e8ddd0;border-radius:12px;overflow:hidden;box-shadow:0 2px 12px rgba(139,111,71,0.1);">
      <div style="position:relative;aspect-ratio:2/3;background:#f5f0e8;">
        ${coverHtml}
        <div style="position:absolute;top:6px;left:6px;display:inline-block;height:auto;font-size:12px;font-weight:700;line-height:1;padding:0px 10px 12px;border-radius:10px;color:#fff;background:${statusInfo.bg};overflow:visible;">${statusInfo.label}</div>
      </div>
      <div style="padding:12px;">
        ...
        ${genreColor ? `<span style="display:inline-block;height:auto;margin-top:6px;font-size:12px;line-height:1;background:${genreColor.bg};color:${genreColor.text};padding:0px 10px 12px;border-radius:10px;overflow:visible;">${escapeHtml(book.genre)}</span>` : ''}
